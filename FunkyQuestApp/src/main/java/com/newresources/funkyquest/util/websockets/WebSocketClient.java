package com.newresources.funkyquest.util.websockets;
...
public class WebSocketClient {
        ...
        mThread = new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    mListener.onDisconnect(Listener.Reason.EOF);
                    mConnected = false;

                } catch (SSLException ex) {
                    // Connection reset by peer
                    Log.d(TAG, "Websocket SSL error!", ex);
                    mListener.onDisconnect(Listener.Reason.CONNECTION_RESET);
                    mConnected = false;

                } catch (Exception ex) {
                    mListener.onError(Listener.ErrorType.CONNECTION,ex);
	                mConnected = false;
                }
            }
        });
                ...
                public void run() {
                    if (mSocket != null) {
                        try {
                            ...
                            mListener.onError(Listener.ErrorType.DISCONNECTION, ex);
                        }
                        mSocket = null;
                    }
	                mListener.onDisconnect(Listener.Reason.MANUAL);
                    mConnected = false;
                }
            });
        }
        ...
        mHandler.post(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    mListener.onError(Listener.ErrorType.DELIVERY, e);
                }
            }
        });
    }
    ...
    public interface Listener {
	    ...
	    public static enum Reason {
		    MANUAL, EOF, CONNECTION_RESET, UNKNOWN
	    }
